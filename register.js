...
system.beforeEvents.startup.subscribe((start) => {
    function register(command, parent) {
        start.customCommandRegistry.registerCommand({ ...command, name: `${Command.prefix ?? 'ccm'}:${command.name}` }, (origin, ...args) => {
            if (command?.required && !command.required(origin))
                return { status: CustomCommandStatus.Failure, message: `§cYou are not allowed to use this command` };
